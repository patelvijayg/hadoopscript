...
for slave in $(cat $HADOOP_CONF_DIR/slaves); do 
   ...
   rsync -avzhe ssh $HADOOP_CONF_DIR/*.* vagrant@$slave:$HADOOP_CONF_DIR/ --delete
   rsync -avzhe ssh $SCRIPT_DIR/ vagrant@$slave:$SCRIPT_DIR/ --delete
done
for slave in $(cat $SPARK_CONF_DIR/slaves); do 
   echo "syncing spark with $slave"
   rsync -avzhe ssh $SPARK_CONF_DIR/*.* vagrant@$slave:$SPARK_CONF_DIR/ --delete
done
